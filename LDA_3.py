...
training_data = financial_data.lda3_data(training_data_dataframe)
...
for i in range(cols):
    ...
    if training_data[c_labels][i] == 0:
        num_decrease += 1
        class_0_decrease[:, num_decrease] = training_data[0:6, i]
    # If the day is an increase
    else:
        num_increase += 1
        class_1_increase[:, num_increase] = training_data[0:6, i]
...
training_data_minus_labels = training_data[0:6][:]
